...
import Spinner from './GiftedSpinner';
    ...
    render(){
        return(
            <View style = {styles.loadingBox}>
                <Spinner style = {{height:30}} size='large'/>
                {this.props.title && <Text style = {{color:'#fff',marginTop:10}}>{this.props.title}</Text>}
            </View>
            )
    }
};
...
export default class Popup extends React.Component{
    ...
    loading(args){
        this.setState({
            isVisible:true,
            loadingStyle:{backgroundColor:'#00000000'},
            loadingBoxStyle:{width:100,backgroundColor:'#00000000'},
            content: (<LoadingContent title = {args} />)
        })
    }
        ...
        if(this.state.isOverlay) {
            return (
                <TouchableWithoutFeedback onPress={() => {
                    ...
                    <View style={[styles.overlay,this.state.loadingStyle]}></View>
                </TouchableWithoutFeedback>
            );
        }
    }
    ...
    _renderContent() {
        return (
            <View style={[styles.tipBoxView,this.state.loadingBoxStyle,this.props.style]}>
                {this.state.content}
            </View>
        );
    }
